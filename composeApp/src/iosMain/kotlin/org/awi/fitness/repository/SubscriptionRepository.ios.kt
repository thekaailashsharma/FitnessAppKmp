...
actual suspend fun revenueCatSwitchPlan(
    ...
    // iOS/StoreKit auto-handles upgrade/downgrade when purchasing another product in the same
    //    subscription group — no replacement mode needed.
    val rawPkg = newPlan.rawPackage ?: return Result.failure(Exception("No package available"))
    return try {
        val purchase = Purchases.sharedInstance.awaitPurchase(rawPkg as Package)
